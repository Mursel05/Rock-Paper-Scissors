...
let index;
let housePicked;
const Game = ({ onSetScore }) => {
  const navigate = useNavigate();
  const userPicked = useParams().type;
  ...
  const [userWin, setUserWin] = useState("player");
  const [houseWin, setHouseWin] = useState("player");
  useEffect(() => {
    index = Math.floor(Math.random() * types.length);
    housePicked = types[index];

    setTimeout(() => {
      setPending("");
      if (result() && result() !== "equal") {
        onSetScore((score) => score + 2);
        setUserWin("player win");
      } else if (!result()) {
        onSetScore((score) => score - 1);
        setHouseWin("player win");
      }
    }, 1000);
  }, []);
  ...
  return (
    <div className={"game " + pending}>
      <div className={userWin}>
        <p>YOU PICKED</p>
        <div style={{ animationDelay: "0s" }} className="circle"></div>
        <div style={{ animationDelay: "0.8s" }} className="circle"></div>
        <div style={{ animationDelay: "1.6s" }} className="circle"></div>
        <div style={{ animationDelay: "2.4s" }} className="circle"></div>
        <div className={userPicked + " item-wrap"}>
          <div className="item">{showItem(userPicked)}</div>
        </div>
      </div>
      {width > 1150 ? (
        <div className="result">
          <h1>
            YOU {result() ? (result() === "equal" ? "DRAW" : "WIN") : "LOSE"}
          </h1>
          <button
            onClick={() => navigate("/")}
      ...
      <div className={houseWin}>
        <p>HOUSE PICKED</p>
        <div style={{ animationDelay: "0s" }} className="circle"></div>
        <div style={{ animationDelay: "0.8s" }} className="circle"></div>
        <div style={{ animationDelay: "1.6s" }} className="circle"></div>
        <div style={{ animationDelay: "2.4s" }} className="circle"></div>
        <div className={housePicked + " item-wrap house"}>
          <div className="item">{showItem(housePicked)}</div>
        </div>
      </div>
      {width <= 1150 ? (
        <div className="result">
